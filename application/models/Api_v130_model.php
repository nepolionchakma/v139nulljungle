    ...
    *****/
    ...
    public function get_single_movie_details_by_id($id=''){
        ...
        if($movie->enable_download == '1'):
            ...
            if(is_demo()):
                $response['download_links']         = $this->get_all_download_links(1000);
            else:
        ...
        if(is_demo()):
            $response['videos']                     = $this->get_all_video_by_movie_id(1000);
        else:
...
//                    $response[$i]['file_url']           = str_replace('.com/','.com/video/',$response[$i]['file_url']);
                elseif($video['file_source'] =='youtube'):
                    $id                                 = $this->common_model->get_youtube_id($video['file_url']);
                    $response[$i]['file_url']           = 'https://www.youtube.com/watch?v='.$id;
                endif;
                $i++;
            endforeach;
        endif;
    ...
    *****/
    ...
    public function get_single_tvseries_details_by_id($id=''){
        ...
        $response['trailler_youtube_source']    = $movie->trailler_youtube_source;
        if(is_demo()):
            $response['season']                     = $this->get_season_episode(1000,'1');
        else:
    ...
    function get_season_episode($videos_id='',$enable_download='0'){
        ...
        if($query->num_rows() > 0):
            ...
            foreach ($seasons as $season):
                ...
                if(is_demo()):
                    $response[$i]['episodes']       = $this->get_episodes_with_all_video_by_movie_id(1000);
                else:
                ...
                if($enable_download == '1'):
                    ...
                    if(is_demo()):
                        $response[$i]['download_links']         = $this->get_all_episode_download_links(1000,1000);
                    else:
...
//                    $response[$i]['file_url']           = str_replace('.com/','.com/video/',$response[$i]['file_url']);
                elseif($episode['file_source'] =='youtube'):
                    $id                                 = $this->common_model->get_youtube_id($episode['file_url']);
                    $response[$i]['file_url']          = 'https://www.youtube.com/watch?v='.$id;
                endif;
                $i++;
            endforeach;
