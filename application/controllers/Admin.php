...
class Admin extends CI_Controller {   
    ...
    function country($param1 = '', $param2 = ''){
        ...
        if ($param1 == 'add') {
            if(is_demo()):
                $this->session->set_flashdata('error', 'Action not allow on demo.');
    ...
    public function movie_upload(){
        ...
        if($source == 'upload'):
            ...
            if($source =='embed'):
                $data['file_url']      = $this->common_model->get_embed_url($this->input->post('url'));
            endif;
    ...
    function manage_live_tv($param1 = '', $param2 = ''){
        ...
        if ($param1 == 'new') {
            ...
            if($data['stream_from']  =='embed'):
                $data['stream_url']     = $this->common_model->get_embed_url($this->input->post('stream_url'));
            endif;
            if($data['stream_from']  =='youtube_live'):
                $id                                 = $this->common_model->get_youtube_id($this->input->post('stream_url'));
                $data['stream_url']           = 'https://www.youtube.com/watch?v='.$id;
            endif;
            $data['description']        = $this->input->post('description');   
            $data['focus_keyword']      = $this->input->post('focus_keyword');   
            $data['meta_description']   = $this->input->post('meta_description');
            ...
            if($data1['source'] =='embed'):
                $data1['url']      = $this->common_model->get_embed_url($data1['url']);
            endif;
            if($data1['source'] =='youtube_live'):
                $id                = $this->common_model->get_youtube_id($data1['url']);
                $data1['url']      = 'https://www.youtube.com/watch?v='.$id;
            endif;
            $data1['quality']       = 'SD';
            ...
            if($data2['source'] =='embed'):
                $data2['url']      = $this->common_model->get_embed_url($data2['url']);
            endif;
            if($data2['source'] =='youtube_live'):
                $id                = $this->common_model->get_youtube_id($data2['url']);
                $data2['url']      = 'https://www.youtube.com/watch?v='.$id;
            endif;
            $data2['quality']       = 'LQ';
